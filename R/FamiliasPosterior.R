FamiliasPosterior <- function (pedigrees, loci, datamatrix, prior, ref = 1, kinship = 0, simplifyMutations = FALSE) 
{
   ...
   #Check that the persons appear in all the pedigrees
   ...
   for (i in loci) {
      if (any(i$alleles<=0))
         stop(paste("ERROR: Problems with allele frequencies in locus", i$locusname))
      if (round(sum(i$alleles), 6)!=1)
         stop(paste("ERROR: Allele frequencies must sum to 1 in locus", i$locusname))

      nAlleles <- length(i$alleles)  
      if (!is.matrix(i$femaleMutationMatrix) | dim(i$femaleMutationMatrix)[1] != nAlleles | 
           dim(i$femaleMutationMatrix)[2] != nAlleles)
      ...
      if (any(as.vector(i$femaleMutationMatrix)<0))
         stop(paste("The female mutation matrix cannot have negative entries in locus", i$locusname))
      if (any(round(apply(i$femaleMutationMatrix, 1, sum), 6) != 1))
         stop(paste("The rows in the female mutation matrix must sum to 1 in locus", i$locusname))
      if (!is.matrix(i$maleMutationMatrix) | dim(i$maleMutationMatrix)[1] != nAlleles | 
           dim(i$maleMutationMatrix)[2] != nAlleles)
      ...
      if (any(as.vector(i$maleMutationMatrix)<0))
         stop(paste("The male mutation matrix cannot have negative entries in locus", i$locusname))
      if (any(round(apply(i$maleMutationMatrix, 1, sum), 6) != 1))
         stop(paste("The rows in the male mutation matrix must sum to 1 in locus", i$locusname))

      lOfArrays <- nAlleles*nAlleles
      simplifyMatrix <- i$simpleMutationMatrices | simplifyMutations
      hasSilentAllele <- (names(i$alleles)[nAlleles]=="silent" | names(i$alleles)[nAlleles]=="Silent")
      result <- .C("AddAlleleSystem", as.integer(nAlleles), 
   	              as.integer(lOfArrays),
		      as.double(i$femaleMutationMatrix),
		      as.double(i$maleMutationMatrix),
		      as.integer(simplifyMatrix),  
 		      as.integer(nAlleles), 
		      as.double(i$alleles), 
		      as.integer(hasSilentAllele), 
		      index = integer(1), 
		      error = integer(1))
      if (result$error>0) 
         stop(paste("ERROR: Problems with input of allele system.", i$locusname))			  
   }	     

   #Input from datamatrix: 
   ...
   for (i in 1:nloci) {
      for (j in 1:npers) {
         ...
         if (!(is.na(A1) && is.na(A2))) {
            #We interpret one missing as a homozygote(!)
            if (is.na(A1)) A1 <- A2
            if (is.na(A2)) A2 <- A1
   ...
   result <- .C("GetProbabilities", 
	  ...
	  likelihoods = double(nloci*npeds), 
	  error = integer(1))
   ...
   likelihoodsPerSystem <- matrix(result$likelihoods, nloci, npeds)
